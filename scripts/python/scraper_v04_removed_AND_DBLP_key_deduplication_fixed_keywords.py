...
KEYWORDS_1 = ["Schema", "Schemas", "Schemata", "Database", "Databases"]
KEYWORDS_2 = ["Evolution", "Change", "Maintenance", "Decay", "Co-Evolution", "History", "Migration", "Update", "Adapt", "Adaptation", "Modify", "Modification"]
...
def query_dblp(query: str, max_results: int = 10000, timeout: int = 200) -> List[Dict[str, Any]]:
    """
    ...
    # Save to file
    with open("query_"+query.replace("\s","_")+"_hits_"+str(len(hits))+".json", "w") as f:
...
if __name__ == "__main__":
    ...
    df = build_dblp_dataframe(KEYWORDS_1, KEYWORDS_2, max_results_per_query=10000)
    # Output to csv
    df.to_csv("10_dblp_keyword_queries_results.csv")
    
    # Filter by year range
    df = filter_by_year_range(df, args.start_year, args.end_year)   
    # Output to csv    
    df.to_csv("20_dblp_years_"+str(args.start_year)+"-"+str(args.end_year)+"_results.csv")
    ...
    # Export to CSV
    deduped_df.to_csv("30_dblp_unique_publications.csv", index=False)
    duplicates_df.to_csv("35_dblp_duplicate_publications.csv", index=False)
